...
export async function handleMessage(ctx, bot) {
  ...
  if (!user || !user.encrypted_token) {
    return ctx.reply(
      'Привет. Я чуть умней записной книжки. Помогаю хранить записи и находить их по запросам. Например, «трактор Клин» или «лучшая шаурма на Рязанском проспекте». Ни к каким твоим данным доступа не имею. Но всё равно совет: не надо хранить чувствительную информацию в интернете.',
      {
        reply_markup: {
          inline_keyboard: [[
